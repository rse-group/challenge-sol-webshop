...
public class ShippingResourceImpl extends ShippingResourceDecorator {
    ...
    @Route(url="call/externalcourier/save")
	...
	@Restricted(permissionName = "Customer")
	@Route(url="call/externalcourier/options")
	public List<HashMap<String, Object>> getShippingOptions(VMJExchange vmjExchange){
		if (vmjExchange.getHttpMethod().equals("OPTIONS")) {
			return null;
		}
		...
		requestBody.put("addressId", addressIdStr);
		List<HashMap<String, Object>> result = ((ShippingServiceImpl) shippingService).getShippingOptions(requestBody);
		return result;
	}

	@Restricted(permissionName = "All")
    @Route(url="call/externalcourier/detail")
    public HashMap<String, Object> getShippingInfo(VMJExchange vmjExchange){
		return shippingService.getShippingInfo();
	}

    @Restricted(permissionName = "All")
    @Route(url="call/externalcourier/list")
